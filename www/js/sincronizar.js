            ...
            base.selecciona("SELECT id FROM pregunta",
                function(tx, resultado){
                    ...
                    for(var i=0; i<=re.length-1; i++){
                        ...
                        if(!esta){
                            cons[cons.length]="INSERT INTO pregunta(id, nombre, respuesta_padre_id, pregunta_tipo_id, obligatoria,	encuesta_id, prioridad, finalizara, encabezado "
                            +")VALUES("
                            +"'"+re[i].id+"','"+re[i].nombre+"','"+re[i].respuesta_padre_id+"','"+re[i].pregunta_tipo_id+"','"+re[i].obligatoria+"',"
                            +"'"+re[i].encuesta_id+"','"+re[i].prioridad+"','"+re[i].finalizara+"','"+re[i].encabezado+"'"
                            +")";
                        }else{
                            ...
                            +", encuesta_id='"+re[i].encuesta_id+"', prioridad='"+re[i].prioridad+"', finalizara='"+re[i].finalizara+"', encabezado='"+re[i].encabezado+"' "
                            +" WHERE id='"+re[i].id+"'; ";
                        }//if
                    }//for
                    for(var k=0; k<=resultado.rows.length-1; k++){
            ...
            base.selecciona("SELECT id FROM respuesta",
                function(tx, resultado){
                    ...
                    for(var i=0; i<=re.length-1; i++){
                        ...
                        if(!esta){
                            cons[cons.length]="INSERT INTO respuesta(id, nombre, nombreObservaciones, pregunta_id, finalizara, esImagen, imagen, ponderacion "
                            +")VALUES("
                            +"'"+re[i].id+"','"+re[i].nombre+"','"+re[i].nombreObservaciones+"','"+re[i].pregunta_id+"','"+re[i].finalizara+"','"+re[i].esImagen+"','"+re[i].imagen+"','"+re[i].ponderacion+"'"
                            +")";
                        }else{
                            cons[cons.length]="UPDATE respuesta SET id='"+re[i].id+"', nombre='"+re[i].nombre+"'"
                            +", nombreObservaciones='"+re[i].nombreObservaciones+"', pregunta_id='"+re[i].pregunta_id+"', finalizara='"+re[i].finalizara+"', esImagen='"+re[i].esImagen+"', imagen='"+re[i].imagen+"', ponderacion='"+re[i].ponderacion+"'"
                            +" WHERE id='"+re[i].id+"'; ";
                        }//if
                    }//for
                    for(var k=0; k<=resultado.rows.length-1; k++){
...
sincronizar.cargarImagenesEncuestas=function(callback){
    ...
    +"  encuesta ", function(tx, resu){
        ...
        +"  respuesta "
        +"WHERE "
        +"  imagen!='' ", function(tx, resu){
            for(var i=0; i<=resu.rows.length-1; i++){
                archDesc[archDesc.length]=resu.rows.item(i).imagen;
            }//for
...
try{
    ...
    function(){//Archivo existe
        ...
        var uri="";
        //alert(archDesc[0]);
        if(archDesc[0].indexOf('respuesta')==-1)
            uri=encodeURI(dominio+"cargas/logotipos/"+archDesc[0]);
        else
            uri=encodeURI(dominio+"cargas/respuestas/"+archDesc[0]);
        //alert(uri);
        ...
        ftr.download(uri,downloadPath,function(e){
            ...
            //alert(uri+"Error al momento de descargar "+downloadPath);
